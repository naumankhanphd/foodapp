...
import { removeCartItemUseCase, updateCartItemUseCase } from "@/lib/cart/use-cases.ts";
...
export async function PATCH(request: NextRequest, { params }: CartItemRouteProps) {
  try {
    ...
    const cart = await updateCartItemUseCase(owner.ownerKey, cartItemId, patch);
...
export async function DELETE(request: NextRequest, { params }: CartItemRouteProps) {
  try {
    ...
    const cart = await removeCartItemUseCase(owner.ownerKey, cartItemId);
    const response = NextResponse.json({ cart });
    return applyCartGuestCookie(response, owner, request);
  } catch (error) {
    return toErrorResponse(error);
