...
function toPriceBadgeText(prices: { largePrice: number; familyPrice: number | null }) {
  const large = `€${formatEuro(prices.largePrice)}`;
  if (prices.familyPrice === null) {
    return large;
  }
  return `${large} / Family €${formatEuro(prices.familyPrice)}`;
}
                            ...
                            {quantity > 0 ? (
                              <div className="relative z-20 inline-flex items-center gap-1 rounded-full border border-[var(--line)] bg-white px-1 py-1">
                                <button
                                  ...
                                  className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-[var(--line)] text-base font-bold text-[#9f430e] hover:bg-[#fff3e6] disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                  -
                                </button>
                                <span className="min-w-5 text-center text-xs font-extrabold text-[#1f1f1f]">{quantity}</span>
                                <button
                                  type="button"
                                  onClick={() => void mutateItemCount(item, "inc")}
                                  disabled={!isCartReady || !isAvailable}
                                  aria-label={`Increase ${item.name}`}
                                  className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[var(--accent)] text-base font-bold text-[var(--accent-ink)] hover:scale-110 disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                  +
                                </button>
                              </div>
