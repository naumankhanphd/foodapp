...
import { hasRequiredRole } from "./policy.ts";
import {
  normalizeEmail,
  parseRequiredRole,
  parseSignupRole,
...
export interface SessionData {
  ...
  user: SanitizedUser & { phoneVerified: boolean };
}
...
function getPool() {
  ...
  if (!g[DB_CACHE_KEY]) {
    g[DB_CACHE_KEY] = mysql.createPool({
      uri: requireDatabaseUrl(),
      connectionLimit: 6,
    });
  }
  return g[DB_CACHE_KEY] as mysql.Pool;
}
...
async function hasColumn(connection: Connection, tableName: string, columnName: string): Promise<boolean> {
...
async function ensureGooglePendingTable(connection: Connection): Promise<void> {
...
async function ensureRoleCatalog(
...
): Promise<{ customerRoleId: number; adminRoleId: number } | null> {
  ...
  const roleRows = rows as Array<{ id: number | string; role_key: string }>;
  const customerRoleId = Number(roleRows.find((entry) => String(entry.role_key) === "CUSTOMER")?.id);
  const adminRoleId = Number(roleRows.find((entry) => String(entry.role_key) === "ADMIN")?.id);
...
async function getUsersNameColumns(connection: Connection) {
...
function normalizeDbUser(row: DbUserRow | undefined): NormalizedUser | null {
  if (!row) {
    return null;
  }
  ...
  return {
    ...
    phoneVerifiedAt: toIsoOrNull(row.phone_verified_at as Date | string | null),
    createdAt: toIsoOrNull(row.created_at as Date | string | null) || new Date().toISOString(),
  };
}

async function findDbUserByEmail(connection: Connection, email: string): Promise<NormalizedUser | null> {
  ...
  if (hasRoleId) {
    ...
    return normalizeDbUser((rows as DbUserRow[])[0]);
  }
...
async function findDbUserById(connection: Connection, userId: string | number): Promise<NormalizedUser | null> {
  const hasRoleId = await hasColumn(connection, "users", "role_id");
...
async function assignRoleForUser(connection: Connection, userId: string, email: string): Promise<void> {
...
async function createDbUser(
  ...
  const userId = String((result as { insertId: number }).insertId);
  if (!hasRoleId) {
...
function hashOneWay(value: string): string {
  return createHash("sha256").update(`${value}:${AUTH_SECRET}`).digest("hex");
}

function randomToken(size: number = 24): string {
  return randomBytes(size).toString("hex");
}

export function getMissingMandatoryProfileFields(userLike: object | null | undefined): string[] {
  const user = (userLike || {}) as Record<string, unknown>;
...
export function hasMandatoryProfileFields(userLike: object | null | undefined): boolean {
  return getMissingMandatoryProfileFields(userLike).length === 0;
}

function resolveLocationPatch(
  input: { lat?: unknown; lng?: unknown },
  fallbackUserLike: { lat?: number | null; lng?: number | null } | null | undefined,
...
export function sanitizeUser(user: NormalizedUser & { phoneVerified?: boolean }): SanitizedUser {
  return {
    id: String(user.id),
    email: user.email,
...
function assertRoleCompatibility(userRole: string, requiredRole: string | null): void {
  if (!requiredRole) {
    return;
  }
...
export async function registerWithPassword(input: Record<string, unknown>): Promise<SanitizedUser> {
...
export async function loginWithPassword(input: Record<string, unknown>): Promise<SanitizedUser> {
  const email = requireEmail(input.email);
  const password = requirePassword(input.password);
...
export async function beginGoogleAuth(input: Record<string, unknown>): Promise<
  | { requiresCompletion: false; user: SanitizedUser }
  | { requiresCompletion: true; pendingToken: string; missingFields: string[] }
> {
  const email = requireEmail(input.email);
  ...
  return await withConnection(async (connection) => {
    ...
    if (existing) {
      ...
      if (hasMandatoryProfileFields(existing as unknown as Record<string, unknown>)) {
        return {
...
export async function completeGoogleProfile(input: Record<string, unknown>): Promise<SanitizedUser> {
...
export async function completeCustomerProfile(input: Record<string, unknown>): Promise<SanitizedUser> {
...
export function createSessionTokenForUser(user: SanitizedUser): string {
...
export function readSessionFromToken(token: string): SessionData | null {
  ...
  const user = {
    ...
    phone: (claims.phone as string | null) || null,
    addressLine1: (claims.addressLine1 as string | null) || null,
    addressCity: (claims.addressCity as string | null) || null,
    lat: toNumberOrNull(claims.lat),
    lng: toNumberOrNull(claims.lng),
    phoneVerified: Boolean(claims.phoneVerified),
    createdAt: (claims.createdAt as string | null) || null,
  };
...
export async function requestPasswordReset(input: Record<string, unknown>): Promise<{ delivered: true; devResetToken?: string }> {
...
export async function resetPassword(input: Record<string, unknown>): Promise<SanitizedUser> {
...
export function listOutboxMessages(): never[] {
  return [];
}
